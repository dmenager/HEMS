...
(defun ex-entropy ()
  (let (bn1 bn2 bn3 bn4 bn5 evidence)
    (setq evidence (make-hash-table :test #'equal))
    ...
    (loop
      ...
      do
	 (cond ((equal "HRPMIN" (rule-based-cpd-dependent-var cpd))
		(setf (gethash (rule-based-cpd-dependent-id cpd) evidence) "120"))
	       #|
	       ((equal "MMHG" (rule-based-cpd-dependent-var cpd))
		(setf (gethash (rule-based-cpd-dependent-id cpd) evidence) "80"))
	       |#))
    ;;(format t "~%evidence: ~A" evidence)
    ;;(format t "~%bn:~%~S" (car eltm*))
    (H[bn] (car eltm*) evidence)
    ))
