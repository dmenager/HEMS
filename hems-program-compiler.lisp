	 ...
	 do
	    ...
	    (loop
		 ...
		 (cond ((eq invariant '>)
			...
			(setq invariant-role1 'term)
			(setq invariant-role2 'term)))
		   ...
		   (with-input-from-string (stream-i var-i)
		     ...
		     (when (funcall invariant num-i num-j)
		       (cond ((funcall invariant num-j num-i)
				...
				(setq new-body (concatenate 'list new-body `(,role1-id = (relation-node ,invariant-role1 :value "CPD_I"))))
				(setq new-body (concatenate 'list new-body `(,role2-id = (relation-node ,invariant-role2 :value "CPD_J"))))
