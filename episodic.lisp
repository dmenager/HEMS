...
(defun py-remember-temporal (eltm temporal-evidence-bn backlinks evidence-bns &key (mode '+) (lr 1) (bicp t) hiddenstatep softlikelihoods (alphas (make-hash-table)) (infernum 0) (singletononly nil))
  (let (alist marginal-alist)
	...
	(remember-temporal eltm temporal-evidence-bn backlinks evidence-bns :mode mode :lr lr :bic-p bicp :hidden-state-p hiddenstatep :soft-likelihoods softlikelihoods :alphas alphas :infer-num infernum :singleton-only singletononly)
      (maphash #'(lambda (k v)
		   (push (list k v) alist))
	       messages)
      (maphash #'(lambda (k v)
