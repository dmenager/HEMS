...
(defun cpd-update-schema-domain (phi1 phi2 &key (q-first-bindings (make-hash-table :test #'equal)))
  (when (and (equal "OBSERVATION_VAR1_209" (rule-based-cpd-dependent-id phi1)))
		     ...
		     (when (and (equal "OBSERVATION_VAR1_209" (rule-based-cpd-dependent-id phi1)))
			  ...
			  (setq svna1 (reverse (cons last-elem (reverse new-svna)))))
			    ...
			    (loop
				 ...
				 ;; if the rule doen't have a condition with the dependent id, make another rule with the negated new condition.
	 ...
	 (loop
         ...
         (loop
           ...
           when (not (member (caar vvbm) vvbms1 :key #'caar :test #'equal))
             do
                (when nil nil (and (equal "ACTION601" (rule-based-cpd-dependent-id phi1)))
...
(defun find-subset-with-max (certain-tog tog junk cpd case-constraints  &key (reject-conditions))
  (loop
    ...
    do
       ...
       (when nil t (and (equal "HAND" (rule-based-cpd-dependent-var cpd)))
  ...
  (labels ((rule-satisfy-case-constraints-p (rule case-constraints &key (avoid nil))
             (loop
               ...
               do
                  ...
                  (return t))))
    (when (and (equal "OBSERVATION_VAR1_209" (rule-based-cpd-dependent-id cpd)))
				    ...
				    ;;(some #'(lambda (rule) (compatible-rule-p new-rule rule cpd cpd)) (union rule-set minimal-rules))
                     ...
                     do
                        (multiple-value-bind (condition condition-block lower-approx conflicts redundancies)
                          ...
                          (when (and (equal "HOLDING1182" (rule-based-cpd-dependent-id cpd)))
...
(defun operate-filter-rules (rules1 rules2 phi1 phi2 new-idents op)
  (labels ((filter-missing-rule (factor1 factor2 rule1 rule2 rule-key seen num-rules)
    ...
    (when (and (equal "OBSERVATION_VAR1_209" (rule-based-cpd-dependent-id phi2)))
...
(defun check-cpd (cpd &key (check-uniqueness t) (check-prob-sum t) (check-counts t) (check-count-prob-agreement t))
  (when t
    (loop
      with check-num-rules = (cond ((> (array-dimension (rule-based-cpd-rules cpd) 0) (reduce #'* (rule-based-cpd-cardinalities cpd)))
  ...
  (labels ((expand-rules (cpd1 cpd2)
	...
	(break))
      ...
      (cond ((eq op '*)
	     ...
	     ;;:initial-contents new-rules)))
	     (check-cpd new-phi :check-uniqueness t :check-prob-sum (if (rule-based-cpd-singleton-p new-phi) nil t) :check-count-prob-agreement (if (rule-based-cpd-singleton-p new-phi) nil t) :check-counts (if (rule-based-cpd-singleton-p new-phi) nil t))
	     ))
      (when (and (equal "OBSERVATION_VAR1_209" (rule-based-cpd-dependent-id phi1)))
