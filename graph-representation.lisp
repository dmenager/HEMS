  ...
  (labels ((rule-satisfy-case-constraints-p (rule case-constraints &key (avoid nil))
             (loop
               ...
               do
                  ...
                  (return t))))
    (when (and (equal "STATE_VAR2_269" (rule-based-cpd-dependent-id cpd)))
...
(defun operate-filter-rules (rules1 rules2 phi1 phi2 new-idents op)
  (labels ((filter-missing-rule (factor1 factor2 rule1 rule2 rule-key seen num-rules)
    ...
    (when (and (equal "STATE_VAR2_269" (rule-based-cpd-dependent-id phi2)))
  ...
  (labels ((expand-rules (cpd1 cpd2)
             (let (var-dif expanded-rules)
               (setq var-dif (block-difference (rule-based-cpd-identifiers cpd2)
                                               ...
                                               :output-hash-p t
                                               :test #'equal))
               (when (and (equal "STATE_VAR2_269" (rule-based-cpd-dependent-id phi1)))
