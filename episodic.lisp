    ...
    (loop
      ...
      do
         (when nil (and print-special* (equal "SIX_483" (rule-based-cpd-dependent-id dummy-match)))
               ;;(format t "~%dummy-match:~%~S~%unmatched q:~%~S" dummy-match (aref q unmatched-q))
	       (format t "~%dummy-match:")
	       (print-cpd dummy-match)
...
(defun new-combine (ep1 ep2 mappings unmatched bindings q-first-bindings)
  ...
  (make-episode :id (episode-id ep1)
		  ...
		  do
                     ...
                     (return new-id-ref-map))
		:observation (cond ((= (array-dimension (car (episode-observation ep2)) 0) 0)
				    (episode-observation ep1))
				   ((and (> (array-dimension (car (episode-observation ep2)) 0) 0)
					 (= (array-dimension (car (episode-observation ep1)) 0) 0))
				    (episode-observation ep2))
				   (t
		...
		:state (cond ((= (array-dimension (car (episode-state ep2)) 0) 0)
			      (episode-state ep1))
			     ((and (> (array-dimension (car (episode-state ep2)) 0) 0)
				   (= (array-dimension (car (episode-state ep1)) 0) 0))
			      (episode-state ep2))
			     (t
			      (new-combine-bns (episode-state ep1) (episode-state ep2) (episode-count ep1) mappings unmatched bindings q-first-bindings)))
                :state-transitions (cond ((= (array-dimension (car (episode-state-transitions ep2)) 0) 0)
					  (episode-state-transitions ep1))
					 ((and (> (array-dimension (car (episode-state-transitions ep2)) 0) 0)
					       (= (array-dimension (car (episode-state-transitions ep1)) 0) 0))
					  (episode-state-transitions ep2))
					 (t
  ...
  (let (pattern base func)
    ...
    (cond ((null x) (values x 0 (list (make-hash-table :test #'equal)) nil nil reject-list -1 func))
	   ...
	   (cond ((and equivalent (null (cdr x)))
		 ...
		 ((and (not equivalent) (null (cdr x)))
		  (let ((new-id (symbol-name (gensym "EPISODE-"))))
		    (setf (episode-id generalized) (episode-id (car x)))
		    (setf (episode-id (car x)) new-id)
                    (push (car x) (cdr (last x)))
                    (setf (car x) generalized)
                    (setf (second x) (list (second x)))
                    (setf (episode-parent (car (second x))) x)
	    ...
	    (t	     
	     (cond ((string-equal type "state-transitions")
		   ...
		   (t
		    (error "uh oh! Unsupported episode field type. Expected \"state\", \"observation\", or \"state-transitions\". Received: ~S" type)))
	     (when nil (string-equal type "observation")
	       (format t "~%~%Did observation node successfully match to existing temporal episode?~%sol:~%~S~%schema bn:" sol)
	       (print-bn bn))      
	     ...
	     (loop
		  ...
		  (when (and q-match (not (member (rule-based-cpd-dependent-id (aref (car bn) q-match))
		    ...
		    (when nil t (and (equal "NVELOCITY" (rule-based-cpd-dependent-var p-copy)))
		    ...
		    (loop
		      for i from q-match below (array-dimension (car bn) 0)
		      when (gethash dep-id (rule-based-cpd-identifiers (aref (car bn) i)))
			do
			   (when (and (equal "A" (rule-based-cpd-dependent-var p-copy)))
			     (format t "~%~%schema cpd before update:")
			     (print-cpd (aref (car bn) i))
			     (format t "~%q-first-bindings:~%~S" q-first-bindings)
			     (format t "~%new-nodes")
			     (mapcar #'print-cpd new-nodes))
			   (setf (aref (car bn) i)
				 (cpd-update-existing-vvms (aref (car bn) i) bindings new-nodes))	     
			   (when (and (equal "A" (rule-based-cpd-dependent-var p-copy)))
		  ...
		  (when soft-likelihoods
		    (loop
		      with soft-cpds = (make-array (array-dimension (car bn) 0))
		      for cpd being the elements of (car bn)
		      for i from 0
		      do
			 (when nil
...
(defun remember-temporal (eltm temporal-evidence-bn backlinks evidence-slices &key (mode '+) (infer-num 0) (lr 1) (bic-p t) (alphas (make-hash-table)) hidden-state-p soft-likelihoods score-only (singleton-only nil))
  (labels ((find-vvbm-by-idx-val (vals vvbms acc)
		    ...
		    do
			 ...
			 when (compatible-rule-p r1 r2 nil nil)
			   do
			      ...
			      (when (< (rule-probability r1) 0)
				(format t "~%malformed rule probability:")
				(print-cpd-rule r1)
				(format t "~%mixture rule:")
				(print-cpd-rule r2)
				(format t "~%alpha: ~d" alpha))
	...
	;;(format t "~%bindings:~%~S~%sol:~%~S" bindings sol)
	...
	|#
		 ...
		 do
		    ...
		    (when (null cond)
		      (setq cond (gethash (gethash (rule-based-cpd-dependent-id cpd)
						   (rule-based-cpd-identifiers cpd))
		 ...
		 do
		    (setq prob (rule-probability rule))
		    (setq c (gethash (rule-based-cpd-dependent-id cpd) (rule-conditions rule)))
		    (when (null c)
...
#| Build list of unique values per cpd-type.
	   ...
	   (pass (messages forward-pass-p)
		 ...
		 (multiple-value-bind (evidence-bn backlinks)
		   ...
		   (when nil print-special*
		     ...
		     (print-bn evidence-bn)
		     #|
		     (format t "~%~%generated by the following observations:")
		     (loop
		       for i being the hash-keys of evidence-slices
			 using (hash-value evidence-hash)
		       do
			  (format t "~%observations for slice: ~d" i)
		     (print-slice evidence-hash))
		     |#
		     ;;(format t "~%backlinks:")
...
(defun py-remember-temporal (eltm temporal-evidence-bn backlinks evidence-bns &key (mode '+) (lr 1) (bicp t) hiddenstatep softlikelihoods (alphas (make-hash-table)) (infernum 0) (singletononly nil) (scoreonly))
  (let (alist marginal-alist)
    (multiple-value-bind (messages marginal-messages)
	(remember-temporal eltm temporal-evidence-bn backlinks evidence-bns :mode mode :lr lr :bic-p bicp :hidden-state-p hiddenstatep :soft-likelihoods softlikelihoods :alphas alphas :infer-num infernum :singleton-only singletononly :score-only scoreonly)
      (maphash #'(lambda (k v)
		   (push (list k v) alist))
	       messages)
      (maphash #'(lambda (k v)
  ...
  (loop
    ...
    do
	 ...
	 (make-temporal-episode-program eltm state-p slice-idx
					...
					:state-transitions nil ;;prog-statements
					:id-ref-hash backlinks
					:prev-st prev-st
					:prev-obs prev-obs
					:prev-act prev-act))
       (setf (aref prog-arr slice-idx) prog-statements)
    finally
       (setq program-statements
	     (reduce #'(lambda (a b)
	...
	do
	   ...
	   (when state-transitions
	     ...
	     (when nil (> (hash-table-count (rule-based-cpd-identifiers (aref (car st-bn) 0))) 1)
		   ...
		   (format t "~%program statements:~%~S~%" state-transitions)
...
(defun pdf-helper (eps parent stream)
  (let* ((ep (car eps))
    ...
    (print-episode-state (episode-observation ep) id "Observation_Model" stream)
    (print-episode-state (episode-state ep) id "State_Model" stream)
    (print-episode-state (episode-state-transitions ep) id "Temporal_Model" stream)
    ;;(format t "~%HERE!!!!")
